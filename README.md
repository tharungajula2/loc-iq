...
Limitations
Confidence scores are illustrative values, not model outputs. There is no scoring engine.
No live API integration. Nothing here calls a real data source.
The scenarios are synthetic and were constructed to show specific failure modes.
Consent classification appears as interface labels only. There is no consent gating logic in code.
The pincode candidates are illustrative Indian metros, not a real geographic resolution index.
Stack
...
Tharun Gajula · Bengaluru, India tharun.gajula.2@gmail.com · LinkedIn · Portfolio
